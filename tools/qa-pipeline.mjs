...
const CONFIG = {
  ...
  maxRounds:    Number(flag('max-rounds', '3')),
  passRate:     Number(flag('pass-rate', '1.0')),
...
Each image MUST contain exactly ONE character/figure. Be extremely strict.

═══ HARD FAIL (instant score 0, pass=false) ═══
Report these issues by name exactly:

- MULTIPLE_CHARS: Image contains more than one distinct character, creature, or figure.
  Count carefully: if you see 2 or more separate bodies, heads, or creatures → FAIL.
  This includes: duplicates, reflections that look like a second figure, pets/companions
  next to the main character, background characters, split/mirrored compositions.
  Even small secondary figures (e.g. a tiny creature on shoulder that has its own distinct body) count.

- MAJOR_CROP: Any part of the character's HEAD, FACE, or TORSO is cut off by the image edge.
  Also fail if more than ~20% of any limb is cut off.
  Check ALL four edges (top, bottom, left, right) carefully.
  If the character extends beyond the frame and you cannot see their full silhouette → FAIL.

- MAJOR_ARTIFACT: Severe visual defects — melted/deformed face, extra limbs (3+ arms/legs),
  fused body parts, unrecognizable anatomy, severe color bleeding/glitches.

═══ SOFT ISSUES (reduce score, may still pass) ═══

- MINOR_CROP: Only the very tips of extremities (weapon tips, wing edges, hair tips, toe tips)
  touch or barely exit the frame. Deduct -10. If more than tips → use MAJOR_CROP instead.
- WHITE_BG: Background is not white/light-gray. Colored or dark background. Deduct -10.
- HELD_ITEMS: Weapon/accessory floating detached or clipping through body. Deduct -15.
- BLURRY: Overall blurry, lacks detail, or unclear silhouette. Deduct -15.

═══ SCORING ═══
Start at 100. Any HARD FAIL → instant score 0, pass=false.
Subtract soft issue points. pass=true only if score >= 70 AND zero hard fails.

IMPORTANT: When in doubt, FAIL. It is better to reject a borderline image than to let a bad one through.

For multiple images return JSON array in order.
{"pass":bool,"score":0-100,"issues":["ISSUE_NAME",...],"suggestion":"fix"}`;
...
async function loadQAReport() {
  try {
    const data = JSON.parse(await readFile(QA_REPORT_PATH, 'utf-8'));
  ...
  for (let bStart = 0; bStart < files.length; bStart += CONFIG.batchSize) {
    ...
    try {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        const hardFails = (result.issues || []).filter(i => ['MULTIPLE_CHARS', 'MAJOR_CROP', 'MAJOR_ARTIFACT'].includes(i));
